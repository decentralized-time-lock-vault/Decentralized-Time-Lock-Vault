...
const MAX_DEPOSITORS_PAGE_SIZE: u32 = 100;
...
impl TimeLockVault {
    ...
    pub fn top_up(
        ...
        if let Some(mut entry) = storage::get_deposit(&env, &depositor, deposit_id) {
            ...
            token_client.transfer(&depositor, &env.current_contract_address(), &added_amount);
    ...
    pub fn cancel_deposit(env: Env, depositor: Address, deposit_id: u32) -> Result<(), VaultError> {
        ...
        if let Some(entry) = storage::get_deposit(&env, &depositor, deposit_id) {
            ...
            let bps_penalty = (entry.amount.saturating_mul(entry.penalty_bps as i128)) / 10_000;
            ...
            if refund > 0 {
                token_client.transfer(&env.current_contract_address(), &depositor, &refund);
            }
    ...
    // ----------------------------------------------------------------
    //  Admin: Batch Emergency Withdraw
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    //  Admin: Pause/Unpause Deposits (Alternative names)
    // ----------------------------------------------------------------
    ...
    pub fn migrate_deposit_to_ledger(
        ...
        if new_unlock_ledger <= current_ledger {
            return Err(VaultError::MigrationNotAllowed);
        }
    ...
    // ----------------------------------------------------------------
    //  Admin: Config Caps / Minimum Fees
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    //  Read-only: Vault status summary
    // ----------------------------------------------------------------
