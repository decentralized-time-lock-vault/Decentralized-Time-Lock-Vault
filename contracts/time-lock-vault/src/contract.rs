...
impl TimeLockVault {
    ...
    pub fn cancel_deposit(env: Env, depositor: Address) -> Result<(), VaultError> {
        ...
        let deposit_id = storage::get_active_deposit_id(&env, &depositor)
    ...
    pub fn batch_emergency_withdraw(
        ...
        for depositor in depositors.iter() {
            let deposit_id = match storage::get_active_deposit_id(&env, &depositor) {
                Some(id) => id,
                None => {
    ...
    pub fn is_admin(env: Env, address: Address) -> bool {
        matches!(storage::get_admin(&env), Some(a) if a == address)
    }
