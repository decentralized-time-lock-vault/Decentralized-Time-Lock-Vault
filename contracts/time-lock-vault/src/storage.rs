...
///
/// Set to 518 400 ledgers ≈ 30 days (518 400 × 5 s = 2 592 000 s).
...
/// the bump to avoid charging callers for storage fees.
pub const BUMP_THRESHOLD: u32 = 518_400;

/// Target TTL (in ledgers) that persistent entries are extended to on every write.
///
/// Computed as `ceil(MAX_LOCK_DURATION_SECS / LEDGER_SECONDS)` ≈ 31 557 600 ledgers
/// (≈ 5.2 years at 5 s per ledger).  This ensures that any deposit created at the
/// maximum allowed lock duration (5 years) cannot expire from storage before its
/// `unlock_time` is reached, even without any subsequent interaction.
